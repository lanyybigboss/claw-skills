...
- **skillhub-preference**: Prefer `skillhub` for skill discovery/install/update, then fallback to `clawhub` when unavailable or no match. Use when users asked about skills, 插件, or capability extension.
- **summarize**: Summarize URLs or files with the summarize CLI (web, PDFs, images, audio, YouTube).
- **tencent-cos-skill**: 腾讯云对象存储(COS)和数据万象(CI)集成技能。
...
### 2. GitHub Token
```bash
./skill-push-token.sh <your-token>
```

### 3. Local Backup
```bash
./local-skill-backup.sh
```

### 4. Web Upload
1. Download `/tmp/claw-skills-backup.tar.gz`
2. Visit https://github.com/lanyybigboss/claw-skills
3. Upload files

## Management Tools

- `skill-manager.sh` - Manage skills (backup, update, install, list)
- `skill-packager.sh` - Package skills into repository
- `skill-github-setup.sh` - GitHub repository setup
- `skill-deploy-all.sh` - Deploy skills to GitHub
